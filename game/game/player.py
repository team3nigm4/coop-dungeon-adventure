...
class Player(entitydrawable.EntityDrawable):
	...
	def __init__(self, args):
		self.colBoxSize = [0.8, 0.8]
		args.append([self.colBoxSize[0] / 2, self.colBoxSize[1] / 2])
		super().__init__(args)

	def update(self):
		if im.input(im.GO_LEFT):
			self.setPos([mam.checkCollisionX(self.pos, -Player.SPEED, self.colBoxSize[0]), self.pos[1]])
		if im.input(im.GO_UP):
			self.setPos([self.pos[0], mam.checkCollisionY(self.pos, Player.SPEED, self.colBoxSize[1])])
